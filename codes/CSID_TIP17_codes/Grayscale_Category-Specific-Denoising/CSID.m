function [y_est, y_est1]  = CSID( noisy, y_est, sigma, lvl, path, ext)
...
top_similar = 16;
im_similar = 16;
% [im_patchs_1, im_patchs_2, im_similar] = search(noisy, path, similar_im);
...
for level=lvl:lvl
    ...
    Nstep = 1;
    %fprintf('patch Size h=%d, r=%d\n', par.win1, par.win2);
    
    for iter  =   1  : par.tot_iter
        
        y_est    =   y_est + par.lamada*(noisy - y_est);
        [~,  Sigma_arr] =   Im2Patch(y_est, noisy, par);
        if (iter == 1 )
        ...
        for row = 1:Nstep:height
            for col = 1:Nstep:width
                ...
                weight = 1;
                y_den(row:row+N1-1, col:col+N2-1)    = y_den(row:row+N1-1, col:col+N2-1)    + rec_ref * weight;
        ...
        y_est1 = 0;
        %         y_est1 = low_rank(1:height,1:width)./weight_total(1:height,1:width);
